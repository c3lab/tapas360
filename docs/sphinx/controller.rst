...
The QualityController is the central component of the adaptive video streaming system. Its goal is to decide the video level,
...
The default ``feedback`` dictionary, that ``Tapas360Player`` updates before calling any new ``calcControlAction()``, is presented in the following table:
