...
def getPage(url, contextFactory=None, *args, **kwargs):
    ...
    try:
        ...
        port = 80
    path = '/'+path
    factory = client.HTTPClientFactory(bytes(url, 'utf-8'), *args, **kwargs)
    factory.noisy = False
    if scheme == 'https':
...
### files usage
def files_usage():
    return int(os.popen('lsof -a -p %d | wc -l' %os.getpid()).read().decode('utf-8'))

### mem usage
def memory_usage(size='rss'):
    """memory sizes: rss, rsz, vsz."""
    return int(os.popen('ps -p %d -o %s | tail -1' %
        (os.getpid(), size)).read().decode('utf-8'))
...
class Logger(object):
    ...
    def logData(self, filename, logdata):
        f = open("{0!s}{1!s}{2!s}".format(self.logs_dir, os.path.sep, filename), "wb")
        f.write(logdata)
        f.close()
    ...
    if options['config']:
        ...
        try:
            if options['config'].endswith('.json'):
                data = open(get_path(options['config'])).read().decode('utf-8')
