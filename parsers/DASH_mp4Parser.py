...
class DASH_mp4Parser(BaseParser):
        ...
        for i in range(0,len_adaptation_set):
            ...
            for j in range(0,len(json_representation)):
                ...
                if isOfType == "video/mp4":
                    ...
                    if "@range" in list(json_representation[j]["SegmentBase"]["Initialization"].keys()):
    ...
    def on_data_received(self, connection, data, level):
        ...
        self.fd[level] = io.StringIO(data)
        #print "level", level
    ...
    def parseAvcC(self, level):
        ...
        sequenceParameters = []
        for i in range(0,numOfSequenceParameterSets):
        ...
        pictureParameters = []
        for i in range(0,numOfPictureParameterSets):
    ...
    def create_gst_codec_data(self, d, level):
        ...
        def AVCDecoderConfigurationRecord():
            ...
            c_d += to_hex(int(reserved, 2),1) #delete 0x
            for i in range(0,internal["numOfSequenceParameterSets"]):
            ...
            for i in range(0,internal["numOfPictureParameterSets"]):
    ...
    def parseSegmentsList(self,level):
        ...
        for i in range(0, len(segment_urls)):
            ...
            byterange = ''
            if "@mediaRange" in list(segment_urls[i].keys()):
                if len(playlist) != 0:
